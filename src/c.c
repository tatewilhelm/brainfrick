#include "c.h"

int transpile_c(struct arguments_t arguments)
{
    if (!arguments.specified_output_filepath)
    {
        arguments.output_filepath = "a.c";
    }
    FILE* file = fopen(arguments.filepath, "r");
    FILE* output_file = fopen(arguments.output_filepath, "w");
    if (fopen(arguments.filepath, "r") == NULL || fopen(arguments.output_filepath, "w") == NULL)
    {
        printf("brainfrick: A fatal error has occured. File does not exist when attempting transpilation read.\n");
        return -1;
    }
    
    if (arguments.infinite_cells) 
    {
        printf("brainfrick: A fatal error has occured. Unlimited Cells is not supported by transpiling.\n");
        return -1;
    }

    

    bool exit = false;
    char program;
    int loop_level = 0;

    // Initial code
    fputs("// Generated by brainfrick, a brainfuck utility\n", output_file);
    fputs("// https://github.com/tatewilhelm/brainfrick\n\n", output_file);

    fputs("#include <stdio.h>\n\n", output_file);
    fputs("char cells[30000];\n", output_file);
    fputs("unsigned int cell_ptr = 0;\n\n", output_file);
    fputs("int main()\n", output_file);
    fputs("{\n", output_file);

    
    while (!exit)
    {
        program = fgetc(file);

        for (int i = 0; i < loop_level; i++)
        {
            fputc('\t', output_file);
        }

        switch (program)
        {
        case '+':
            fputs("// +\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("cells[cell_ptr]++;\n", output_file);
            break;
        case '-':
            fputs("// -\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("cells[cell_ptr]--;\n", output_file);
            break;
        case '>':
            fputs("// >\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("if (cell_ptr == 30000)\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("{\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("\tcell_ptr = 0;\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("} else {\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("\tcell_ptr++;\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("}\n", output_file);
            break;
        case '<':
            fputs("// <\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("if (cell_ptr == 0)\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("{\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("\tcell_ptr = 30000;\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("} else {\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("\tcell_ptr--;\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("}\n", output_file);
            break;
        case '.':
            fputs("// .\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("printf(\"%c\", cells[cell_ptr]);\n", output_file);
            break;
        case ',':
            fputs("// ,\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("scanf(\"%c\", &cells[cell_ptr]);\n", output_file);
            break;
        case '[':
            fputs("// [\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("while (cells[cell_ptr])\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("{\n", output_file);
            loop_level++;
            break;
        case ']':
            fputs("// ]\n", output_file);
            for (int i = 0; i < loop_level; i++)
            {
                fputc('\t', output_file);
            }
            fputs("}\n", output_file);
            loop_level--;
            break;    
        case EOF:
            if (arguments.return_ending_cell)
            {
                fputs("return cells[cell_ptr];\n", output_file);
            }
            fputs("}\n", output_file);
            exit = true;
            break;
        default:
            break;
        }
    }

    fclose(file);
    fclose(output_file);

    return 0;

}